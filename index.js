...
navigator.geolocation.getCurrentPosition(location => {
    fetch("https://api.weatherapi.com/v1/forecast.json?key=f7d63b9a0a45493e9cb52439231302&q= " + location.coords.latitude + "," + location.coords.longitude + "&aqi=yes&days=8")
...
const Weatherfunction = (city) => {
    fetch("https://api.weatherapi.com/v1/forecast.json?key=f7d63b9a0a45493e9cb52439231302&q= " + city + "&aqi=yes&days=8")
        .then((res) => { return res.json() })
        .then((data) => {
            Displayweather(data)
            console.log(data)
...
const autocomplete = (city) => {
    console.log(city);
    fetch("https://api.weatherapi.com/v1/search.json?key=f7d63b9a0a45493e9cb52439231302&&q=" + city)
        .then((res) => { return res.json() })
        .then((data) => {
...
const Displayweather = (data) => {
    ...
    const { avghumidity, maxwind_kph, uv, daily_chance_of_rain } = data.forecast.forecastday[0].day;
    ...
    document.getElementById("today").innerHTML = weekday[day.getDay()] + ", " + month[day.getMonth()] + " " + date.slice(8, 10) + " " + date.slice(0, 4);
    ...
    ((data.forecast.forecastday).slice(1, 8)).map(data => {
        ...
        value = value + `<div class=" rounded rounded-3 d-flex  flex-column justify-content-evenly align-items-center bg-white shadow shadow-md border border-1 forecast-card ">
            <span class="fs-6 fw-bold">${weekday[d.getDay()]}</span>
            <img width="40px" src="./icons/day/${data.day.condition.code}.svg" alt="night-drizzle" />
            <span class="base-text fw-bold text-black-50">${data.day.maxtemp_c + " °C"}</span>
        </div >`
    })
